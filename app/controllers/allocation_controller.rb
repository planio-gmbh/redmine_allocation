class AllocationController < ApplicationController
  ...
  def by_user
    ...
    subproject_users = User.active.where(@project.project_condition(true)).
    ...
    if user_group_field.present? and project_group_field.present?
      ...
      group_users = User.active.
        where(
        "#{CustomField.table_name}.id = ? AND #{CustomValue.table_name}.value = ?", user_group_field, quoted_group
        ).
